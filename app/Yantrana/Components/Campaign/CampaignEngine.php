...
class CampaignEngine extends BaseEngine implements CampaignEngineInterface
{
        ...
        $requireColumns = [
            ...
            'scheduled_status' => function ($rowData) use (&$timeNow) {
                ...
                if(Carbon::parse($rowData['scheduled_at']) < $timeNow) {
                    ...
                    if(($rowData['queue_pendings_messages_count'] or $rowData['queue_processing_messages_count']) and $rowData['message_log_count']) {
                    ...
                    } elseif(!$rowData['queues_pending_messages_count'] and !$rowData['message_log_count']) {
                        $statusText = __tr('NA');
                    }
                }
                return $statusText;
    ...
    public function processCampaignDelete($campaignIdOrUid)
    {
        ...
        // if failed to delete
        return $this->engineFaisledResponse([], __tr('Failed to delete Campaign'));
    }
    /**
    ...
    *---------------------------------------------------------------- */
    ...
    public function prepareCampaignUpdateData($campaignIdOrUid)
    {
        ...
        // if record found
        return $this->engineSuccesssResponse($campaign->toArray());
    }
    ...
    public function processGenerateCampaignExecutedReport($campaignUid = null)
    {
        ...
        $writer->writeSheetRow($sheet1, [$campiaignData['template_language']], $styles1);
        //scheduled_at Row
    ...
    public function processGenerateQueueLogCampaignReport($campaignUid = null)
    {
        ...
        $this->campaignRepository->fetchCampaignQueueLogDataLazily($campaign->_id, function (object $queueLogEntry) use (&$writer, &$sheet1, &$styles3) {
            ...
            $writer->writeSheetRow($sheet1, [
                ($contactsData['first_name'] ?? '') ? ($contactsData['firsts_name'] ?? '') . ' ' . ($contactsData['last_name'] ?? '') : ($contactsData['last_name'] ?? ''),
